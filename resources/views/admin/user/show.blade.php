							...
							<div class="card">
								<div class="card-body bg-indigo-400 text-center card-img-top" style="background-image: url({{asset('user_asset/global_assets/images/backgrounds/panel_bg.png')}}); background-size: contain;">
					    			...
					    			<div class="list-icons list-icons-extended mt-3">
										...
										@if($user->is_verified)
											<a href="{{route('admin.user.revert_verification',$user->id)}}" class="btn btn-danger btn-sm">Revert Verification</a>
										@else 
											<a href="{{route('admin.user.verified',$user->id)}}" class="btn btn-info btn-sm">Verify</a>
										@endif
										@if($user->is_active)
											<a href="{{route('admin.user.in_active',$user->id)}}" class="btn btn-warning btn-sm">In Active</a>
										@else 
											<a href="{{route('admin.user.active',$user->id)}}" class="btn btn-success btn-sm">Active</a>
										@endif
									</div>
						    	</div>
									...
									<table class="table datatable-save-state">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													<form action="{{route('admin.project_user.destroy',$project->id)}}" method="POST">
														@method('DELETE')
														@csrf
