							...
							<div class="card">
								<div class="card-body bg-indigo-400 text-center card-img-top" style="background-image: url({{asset('user_asset/global_assets/images/backgrounds/panel_bg.png')}}); background-size: contain;">
					    			...
					    			<div class="list-icons list-icons-extended mt-3">
										...
										@if($user->is_verified)
											<a href="{{route('executive.user.revert_verification',$user->id)}}" class="btn btn-danger btn-sm">Revert Verification</a>
										@else 
											<a href="{{route('executive.user.verified',$user->id)}}" class="btn btn-info btn-sm">Verify</a>
										@endif
										@if($user->is_active)
											<a href="{{route('executive.user.in_active',$user->id)}}" class="btn btn-warning btn-sm">In Active</a>
										@else 
											<a href="{{route('executive.user.active',$user->id)}}" class="btn btn-success btn-sm">Active</a>
										@endif
									</div>
						    	</div>
								...
								<div class="card-body p-0">
									<ul class="nav nav-sidebar mb-2">
										...
										<li class="nav-item">
											<a href="{{route('executive.user.index')}}" class="nav-link" data-toggle="tab">
					...
					<div class="tab-content w-100 overflow-auto">
						<div class="tab-pane fade active show" id="profile">
							...
							<div class="card">
								...
								<div class="card-body">
									<form action="{{route('executive.user.update',$user->id)}}" method="post" enctype="multipart/form-data" >
										@method('PUT')
										@csrf
					    ...
					    {{-- <div class="tab-pane fade" id="inbox">
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<form action="{{route('executive.project_user.store')}}" method="post" enctype="multipart/form-data" >
										@csrf
										<div class="form-group">
									...
									<table class="table datatable-save-state">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													<form action="{{route('executive.project_user.destroy',$project->id)}}" method="POST">
														@method('DELETE')
														@csrf
